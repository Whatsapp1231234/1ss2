...
const smsAvisoAG = () => { return `╰⊱⚠️⊱ *ПРЕДинформацияУПРЕЖДЕНИЕ* ⊱⚠️⊱╮\n\n` }
...
const smsJBConexionClose2 = () => { return `${lenguaje['smsAvisoFG']()}🔴*ВАШЕ УСТРОЙСТВО ПОДКЛЮЧЕНОED*\n\n*ВЫ ДОЛЖНЫ ПОДКЛЮЧИТЬСЯ СНОВА, ЧТОБЫ ИСПОЛЬЗОВАТЬ:\n#удаление соединения`}
const smsreenvia = () => { return `*🟢 RESEND COMMAND...*`}
const smsJBPrincipal = () => { return `${lenguaje['smsAvisoAG']()}🔵 *ЧТОБЫ СТАТЬ ВСПОМОГАТЕЛЬНЫМ БОТОМ, ПЕРЕЙДИТЕ НА ОСНОВНОЙ НОМЕР*\n*ღ Перейдите по следующей ссылке:*\n`}
...
//comandos+18-menu.js
const smsList1 = () => { return -  `\nУ меня не достаточно `}
...
const smsContacto2 = () => { return 'Я создатель ' + packname + 'если у вас есть какие-либо вопросы, вы можете задать их мне ✌️'}
const smsContacto3 = () => { return '👑 создатель официальный контакт 🐈'}
...
smsJBCerrarS, smsJBErr, smsContAdult, smsList1, smsList2, smsList3, smsList4, smsList5, smsList6, smsConj, smsFras, smsContacto1, smsContacto2, smsContacto3, smsContacto5,
